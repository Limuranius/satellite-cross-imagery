...
def dist(i1: Info, i2: Info):
    c1 = i1.center_coord()
    c2 = i2.center_coord()
    return math.sqrt((c1[0] - c2[0]) ** 2 + (c1[1] - c2[1]) ** 2)

pairs.sort(key=lambda pair: dist(*pair))
print([dist(*pair) for pair in pairs])


# Filtering by latitude
# pairs = [pair for pair in pairs if abs(pair[0].center_coord()[1]) > MIN_LATITUDE]
pairs = [pair for pair in pairs if abs(pair[0].center_coord()[1]) > 60]
...
dts = [datetime.datetime.fromisoformat(s) for s in """
2024-01-27 18:15:00
2024-03-10 18:20:00
2024-04-12 13:05:00
2024-01-12 12:45:00
...
# show_mersi_previews()
# visuals.map_2d.show_image_boxes(list_modis + list_mersi).show_in_browser()
